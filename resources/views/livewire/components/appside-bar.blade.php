<div class="relative hidden md:block" wire:ignore>
    ...
    <nav id="sidebar-nav"
        class="sticky bottom-0 left-0 top-0 z-40 flex h-screen flex-col items-center justify-between rounded-r-xl bg-white p-2 font-medium text-gray-700 drop-shadow-xl dark:bg-[#010417] lg:p-3">
        {{-- app name and logo --}}
        <section class="items-center-remover flex flex-col">
            <a wire:navigate href="{{ route('index') }}" class="relative flex items-center p-2">
            ...
            <div class="items-center-remover flex flex-col items-center gap-3 pt-2 text-sm">
                @auth
                    <a wire:navigate href="{{ route('user-notification') }}"
                        class="{{ request()->routeIs('user-notification') ? ' bg-slate-200  dark:bg-slate-800' : 'hover:bg-slate-300 dark:hover:bg-slate-700  duration-500 ease-in-out' }} relative flex rounded-xl p-2"
                        id="notification">
                        <div class="relative w-fit">
                            <svg class="h-6 w-6 text-gray-700 dark:text-slate-50" fill="currentColor" viewBox="0 0 24 24"
                            ...
                            @if ($numberOfNotification > 0)
                                <div
                                    class="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full border-2 border-white bg-gray-700 text-gray-100 dark:border-slate-500 dark:bg-slate-50 dark:text-gray-800">
                                    <p class="text-[8px]">{{ $numberOfNotification <= 9 ? $numberOfNotification : '9+' }}
                                    </p>
                                </div>
                            @endif
                        </div>
                        <p class="hideName block pl-2 dark:text-slate-50 md:hidden">Notification</p>
                    </a>
                    {{-- message --}}
                    {{-- <a wire:navigate href="{{ route('') }}"
                        class="{{ request()->routeIs('') ? ' bg-slate-200 dark:bg-slate-800' : 'hover:bg-slate-300  duration-500 ease-in-out' }} flex items-center rounded-xl p-2">
                        <div class="relative">
                            ...
                            <div
                                ...
                                <p class="text-[8px]">3</p>
                            </div>
                        </div>
                        <p class="   dark:text-slate-50  hideName block pl-2 md:hidden">Messages</p>
                    </a> --}}
                @endauth
                {{-- HOME --}}
                <a wire:navigate href="{{ route('home') }}"
                    class="{{ request()->routeIs('home') ? ' bg-slate-200 dark:bg-slate-800' : 'hover:bg-slate-300 dark:hover:bg-slate-700   duration-500 ease-in-out' }} flex items-center rounded-xl p-2"
                    id="home">
                    <div class="relative">
                        <svg class="h-6 w-6 text-gray-700 dark:text-slate-50" fill="currentColor" viewBox="0 0 24 24">
                            <path
                    ...
                    <p class="hideName block pl-2 dark:text-slate-50 md:hidden">Home</p>
                </a>
                {{-- search --}}
                <a wire:navigate href="{{ route('user-search') }}"
                    class="{{ request()->routeIs('user-search') ? ' bg-slate-200 dark:bg-slate-800' : 'hover:bg-slate-300 dark:hover:bg-slate-700   duration-500 ease-in-out' }} flex items-center rounded-xl p-2"
                    id="search">
                    <div class="relative">
                        <svg class="h-6 w-6 text-gray-700 dark:text-slate-50" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <p class="hideName block pl-2 dark:text-slate-50 md:hidden">Search</p>
                </a>
                {{-- catalogue --}}
                <a wire:navigate href="{{ route('user-catalogue') }}"
                    class="{{ request()->routeIs('user-catalogue') ? ' bg-slate-200 dark:bg-slate-800' : 'hover:bg-slate-300 dark:hover:bg-slate-700   duration-500 ease-in-out' }} flex items-center rounded-xl p-2"
                    id="catalogue">
                    ...
                    <p class="hideName block pl-2 dark:text-slate-50 md:hidden">Catologue</p>
                </a>
                {{-- bookmark --}}
                <a wire:navigate href="{{ route('user-bookmark') }}"
                    class="{{ request()->routeIs('user-bookmark') ? ' bg-slate-200 dark:bg-slate-800' : 'hover:bg-slate-300 dark:hover:bg-slate-700   duration-500 ease-in-out' }} flex items-center rounded-xl p-2"
                    id="bookmark">
                    <div class="relative">
                        <svg class="h-6 w-6 text-gray-700 dark:text-slate-50" fill="currentColor" viewBox="0 0 24 24">
                            <path
                    ...
                    <p class="hideName block pl-2 dark:text-slate-50 md:hidden">Bookmark</p>
                </a>
                @auth
                    {{-- user/profile --}}
                    <a wire:navigate href="{{ route('user-profile', ['username' => auth()->user()->username]) }}"
                        class="{{ request()->routeIs('user-profile') && request()->route('username') === auth()->user()->username ? ' bg-slate-200 dark:bg-slate-800' : 'hover:bg-slate-300 dark:hover:bg-slate-700   duration-500 ease-in-out' }} flex items-center rounded-xl p-2"
                        id="user">
                        <div class="relative">
                            <svg class="h-6 w-6 text-gray-700 dark:text-slate-50" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <p class="hideName block pl-2 dark:text-slate-50 md:hidden">User</p>
                    </a>
                @endauth
            </div>
        </section>
        ...
        <div class="items-center-remover flex flex-col gap-3 pt-2 text-sm">
            ...
            <button
                class="{{ request()->routeIs('') ? ' bg-slate-200 dark:bg-slate-800' : 'hover:bg-slate-300 dark:hover:bg-slate-700   duration-500 ease-in-out' }} relative flex rounded-xl p-2"
                id="darkmode">
                <div class="relative w-fit">
                    <svg class="h-6 w-6 text-gray-700 dark:text-slate-50" fill="currentColor" viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg">
            ...
            <a wire:navigate href="{{ route('help-and-support') }}"
                class="{{ request()->routeIs('help-and-support') ? ' bg-slate-200 dark:bg-slate-800' : 'hover:bg-slate-300 dark:hover:bg-slate-700   duration-500 ease-in-out' }} relative flex rounded-xl p-2"
                id="helpSupport">
                <div class="relative w-fit">
                    <svg class="h-6 w-6 text-gray-700 dark:text-slate-50" fill="currentColor" viewBox="0 0 24 24"
                        xmlns="http://www.w3.org/2000/svg">
                ...
                <p class="hideName block pl-2 dark:text-slate-50 md:hidden">Help</p>
            </a>
            @auth
                {{-- logout --}}
                <form action="{{ route('user.logout') }}"
                    class="flex cursor-pointer rounded-xl p-2 duration-500 ease-in-out hover:bg-slate-300 dark:hover:bg-slate-700"
                    ...
                    <p class="hideName block pl-2 dark:text-slate-50 md:hidden">LogOut</p>
                </form>
            @endauth
        </div>
    </nav>
