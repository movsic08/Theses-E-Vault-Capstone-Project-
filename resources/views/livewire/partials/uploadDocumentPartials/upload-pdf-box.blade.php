  <div x-data="{ uploadPDF: false }" x-show="uploadPDF" x-on:open-pdf.window="uploadPDF = true"
      ...
      <div x-data="{ uploading: false, progress: 0 }" x-on:livewire-upload-start="uploading = true; progress = 0"
          ...
          <div class="flex h-fit w-full flex-col items-center rounded-b-xl bg-white px-2 py-4">
              <div class="container">
                  <div class="mt-5 rounded-md bg-gray-100 p-2 text-gray-600">Please be advised
                      that the required file must be in PDF format. Additionally, kindly note that the PDF file size
                      should not exceed <strong class="text-red-500">100MB.</strong> Thank you for your cooperation.
                  </div>
                  <div>
                      @error('user_upload')
                  ...
                  <div class="mt-7 flex w-full flex-col items-center justify-center gap-2 font-semibold md:flex-row">
                      <div @click="uploadPDF = false"
                          class="w-full cursor-pointer rounded-lg border border-blue-800 bg-blue-100 px-3 py-2 align-middle text-blue-800 duration-300 hover:bg-blue-800 hover:text-white">
                          Close</div>
                      <label for="uploadFile"
                          class="w-full rounded-lg border-primary-color bg-primary-color px-3 py-2 text-center text-white duration-300 hover:cursor-pointer hover:border-blue-900 hover:bg-blue-900">Upload
