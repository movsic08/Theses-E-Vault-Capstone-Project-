         ...
         <div class="container">
             ...
             @foreach ($replyComments as $replyCommentsItem)
                 @if ($replyCommentsItem->parent_id == $commentsItem->id)
                                 ...
                                 <div class="flex w-full justify-start">
                                     <ul class="flex gap-2 text-xs">
                                         @if (auth()->check())
                                             ...
                                             @endif
                                             @if ($replyCommentsItem->user_id !== auth()->user()->id)
                                                 <li wire:click='toggleReplyToRepliedBox( {{ $commentsItem->id }}, {{ $replyCommentsItem->user_id }})'
                                                     class="cursor-pointer transition duration-200 ease-in-out hover:font-semibold">
                                                     Reply
                                                 </li>
                                             @endif
                     ...
                     {{-- replying to reply form  &&  === $replyCommentsItem->id --}}
                     @if ($showReplyingBox && $targetReply === $commentsItem->id)
                     ...
                     {{-- replies to reply --}}
                     @foreach ($repliesToReplyComments as $repliesToReplyCommentsItem)
                         @if ($repliesToReplyCommentsItem->parent_id == $replyCommentsItem->parent_id)
                             @php
                                 $commentAuthorDetailsReply = App\Models\User::where('id', $repliesToReplyCommentsItem->user_id)->first();
                                 $fullnameReplied = $commentAuthorDetailsReply->first_name . ' ' . $commentAuthorDetailsReply->last_name;
                             @endphp
                             <div class="ml-12 mt-1.5 rounded-xl bg-white px-4 py-2 drop-shadow-md"
                                 ...
                                 <div class="flex w-full">
                                     ...
                                     <div class="w-full">
                                         <div class="flex w-full flex-col leading-tight">
                                             <div class="flex w-full justify-between">
                                                 <div>
                                                     <strong>{{ $fullnameReplied }}</strong>
                                                     @php
                                                         //  $parentID = App\Models\DocuPostComment::where('id', $repliesToReplyCommentsItem->reply_parent_author)->first();
                                                         $detailOfMainCommentor = App\Models\User::where('id', $repliesToReplyCommentsItem->reply_parent_author)->first();
                                                         if ($detailOfMainCommentor) {
                                                             $fullNameMainReplied = $detailOfMainCommentor->first_name . ' ' . $detailOfMainCommentor->last_name;
                                                         } else {
                                                             $fullNameMainReplied = 'user';
                                                         }
                                                     @endphp
                                                     <small> replied to {{ $fullNameMainReplied }}</small>
                                                 </div>
                                                 <section class="flex gap-1">
                                                     <div class="flex items-center gap-1">
                                                         @if ($repliesToReplyCommentsItem->created_at != $repliesToReplyCommentsItem->updated_at)
                                                             <small class="text-xs">Edited</small>
                                                             <div class="h-1 w-1 rounded-full bg-slate-700"></div>
                                                         @endif
                                                     </div>
                                             ...
                                             <div class="flex gap-2">
                                                 @php
                                                     $commentorInfo = \App\Models\User::find($repliesToReplyCommentsItem->user_id);
                                                     $bachelorDegree = \App\Models\BachelorDegree::find($commentorInfo->bachelor_degree);
                                                     // $commentDate = Carbon\Carbon::parse($commentAuthorDetails->created_at)->setTimezone('Asia/Manila');
                                                     // $timeAgo = $commentDate->diffForHumans();
                                                 @endphp
                                         ...
                                         <div class="my-2 w-full rounded-md py-1">
                                             @if ($editingCommentId === $repliesToReplyCommentsItem->id)
                                                 <section class="flex w-full flex-row items-start gap-2">
                                                     <div class="w-full">
                                                         <x-input-field wire:model="editedComment" type="text"
                                                             class="w-full" />
                                                     ...
                                                     <button
                                                         class="h-9 w-fit rounded-md bg-blue-700 px-2 py-1 font-semibold text-white duration-300 ease-in-out hover:bg-primary-color"
                                                         wire:click="updateComment({{ $repliesToReplyCommentsItem->id }})">Save</button>
                                                     <button
                                                         class="h-9 w-fit rounded-md bg-blue-700 px-2 py-1 font-semibold text-white duration-300 ease-in-out hover:bg-primary-color"
                                                         wire:click="cancelEditing">Cancel</button>
                                                 </section>
                                             @else
                                                 <p class="">{{ $repliesToReplyCommentsItem->comment_content }}
                                                 </p>
                                             @endif
                                         </div>
                                         <div class="flex w-full justify-start">
                                             <ul class="flex gap-2 text-xs">
                                                 @if (auth()->check())
                                                     @if ($repliesToReplyCommentsItem->user_id === auth()->user()->id)
                                                         <li wire:click='editComment({{ $repliesToReplyCommentsItem->id }})'
                                                             class="cursor-pointer transition duration-200 ease-in-out hover:font-semibold">
                                                             Edit
                                                         </li>
                                                         <li wire:click='deleteComment({{ $repliesToReplyCommentsItem->id }})'
                                                             class="cursor-pointer transition duration-200 ease-in-out hover:font-semibold">
                                                             Delete
                                                         </li>
                                                     @endif
                                                     @if ($repliesToReplyCommentsItem->user_id !== auth()->user()->id)
                                                         <li {{-- wire:click='toggleReplyToRepliesOfRepplyBox( )' --}}
                                                             wire:click='toggleReplyToRepliesOfReplyBox(  {{ $repliesToReplyCommentsItem->id }}, {{ $commentsItem->id }}, {{ $repliesToReplyCommentsItem->user_id }})'
                                                             class="cursor-pointer transition duration-200 ease-in-out hover:font-semibold">
                                                             Reply
                                                         </li>
                                                     @endif
                             ...
                             {{-- replying to reply of repliers form   --}}
                             @if ($showReplyingBox && $targetReplies == $repliesToReplyCommentsItem->id)
                                 <div
                                     class="col-span-4 ml-12 mt-3 rounded-lg bg-white p-2 drop-shadow-lg lg:col-span-3">
                                     <form wire:submit.prevent='createReplyToReplies'>
                                         <div class="flex w-full items-center justify-center gap-2">
                                             <textarea wire:model.live='replyCommentContent' wire:keydown.enter="createReplyToReplies"
                                                 class="w-full rounded-lg border-2 border-primary-color bg-gray-100 p-2" name=""
                                             ...
                                             <input
                                                 class="w-fit cursor-pointer rounded-lg bg-primary-color p-2 text-white"
                                                 type="submit" value="Reply">
                                         </div>
                                         @error('replyCommentContent')
                                             <small class="my-1 text-red-500">{{ $message }}</small>
