...
class ViewDocuPost extends Component
{
    ...
    public function deleteComment($id)
    {
        ...
        if (!$isDeleted) {
            request()
                ->session()
                ->flash('error', 'Deleting comment failed, contact developer!');
        }
        return;
    }
    ...
    public function toggleBookmark()
    {
        if (!auth()->check()) {
            ...
            $checkInfoData = empty($this->authenticatedUser->last_name && $this->authenticatedUser->first_name && $this->authenticatedUser->last_name && $this->authenticatedUser->address && $this->authenticatedUser->phone_no && $this->authenticatedUser->student_id && $this->authenticatedUser->bachelor_degree);
            if ($checkInfoData) {
                request()
                    ->session()
                    ->flash('message', 'Account information details are incomplete, fill out now here.');
            } else {
                if ($this->authenticatedUser->is_verified == 0) {
                    request()
                        ->session()
                        ->flash('message', 'Ve rify your account now, to enjoy the full features for free.');
                } else {
    ...
    public $showReplyingBox = false,
        $targetMainIDForReplyingReply,
        $mainCOmmentAUthor;
