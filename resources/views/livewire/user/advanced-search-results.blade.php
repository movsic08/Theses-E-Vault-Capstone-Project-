        ...
        <div class="relative mb-8 flex flex-col gap-4 lg:flex-row lg:gap-8">
            <section class="flex w-full flex-col gap-4">
                <div class="flex w-full items-center justify-between text-primary-color">
                    ...
                    <strong class="ml-1 whitespace-nowrap text-primary-color">{{ $resultsCount }}
                        @if ($resultsCount <= 1)
                            Result found
                        @else
                            Results found
                ...
                @if (!$results->isEmpty())
                    <div class="relative flex w-full flex-col gap-5">
                        <div wire:loading
                            wire:target='searchNewDocu, sort_by, items, docuTypeInput, language, dateMYOnly, datepicker, dateMYOnly, dateRangeStart, dateRangeEnd'
            ...
            <div class="mb-4 h-[20rem] w-full lg:sticky lg:right-0 lg:top-[3.5rem] lg:w-1/2">
                <div class="lg:mt mb-4 flex w-full flex-col gap-2 rounded-lg bg-white p-3 drop-shadow">
                    <strong class="flex w-full items-center justify-start gap-1 uppercase text-primary-color"><svg
                            class="h-6" fill="currentColor" viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M4.68 4.68a.6.6 0 0 1 .6-.6h14.4a.6.6 0 0 1 .6.6v2.4a.6.6 0 0 1-.154.4l-5.246 5.83v5.77a.6.6 0 0 1-.41.569l-3.6 1.2a.6.6 0 0 1-.79-.569v-6.97L4.833 7.48a.6.6 0 0 1-.153-.4v-2.4Zm1.2.6v1.57l5.246 5.83a.6.6 0 0 1 .154.4v6.367l2.4-.799V13.08a.6.6 0 0 1 .153-.4l5.247-5.83V5.28H5.88Z">
                            </path>
                        </svg>Filter</strong>
                    {{-- documenty type --}}
                    <div class="flex w-full flex-row items-center justify-start gap-4">
                        <div class="flex items-center gap-2">
                            <x-label-input>Type</x-label-input>
                            <select wire:model.live='docuTypeInput'
                                ...
                                @if ($docuTypeInput !== 'all')
                                    <option value="all">all</option>
                                @endif
                                <option selected value="{{ $docuTypeInput }}">{{ $docuTypeInput }}</option>
                                @foreach ($documentTypes as $item)
                                    @if ($item !== $docuTypeInput)
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <select wire:live='language'
                                class="block w-fit rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500">
                                @if ($language !== 'all')
                                    <option value="all">all</option>
                                @endif
                                <option selected value="{{ $language }}">{{ $language }}</option>
                                @foreach ($documentLanguage as $item)
                                    @if ($item !== $language)
                                        <option value="{{ $item }}">{{ $item }}</option>
                                    @endif
                                @endforeach
                    ...
                    <div class="flex w-full flex-col items-start justify-between gap-2" x-data="{ dateOption: '{{ $datepicker }}' }">
                        {{-- date picker --}}
                        ...
                        <div x-show="dateOption === 'month-and-year-only'"
                            ...
                            class="relative flex w-full transition-all">
                            <div class="flex items-center justify-center gap-2">
                                <x-label-input class="w-fit whitespace-nowrap">Month and Year</x-label-input>
                        ...
                        <div x-show="dateOption === 'date-range'" x-transition:enter.duration.400ms.delay.300ms
                            x-transition:leave.duration.200ms class="flex w-full items-center transition-all">
                        ...
                        <section class="flex w-full flex-row items-start justify-between gap-2">
                            {{-- items --}}
                            <div
                        ...
                        <div class="flex w-full items-end justify-end">
                            <button wire:click='searchNewDocu'
                                class="rounded-lg bg-primary-color px-2 py-1 font-medium text-white">Apply</button>
                        </div>
                    </div>
                </div>
